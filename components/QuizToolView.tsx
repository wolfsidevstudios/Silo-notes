...
import { BackIcon, QuizIcon } from './icons';
...
const QuizQuestionComponent: React.FC<{ question: QuizQuestion }> = ({ question }) => {
  ...
  const getOptionClass = (option: string) => {
    ...
    if (option === selectedAnswer) return 'bg-red-100 border-red-500';
    return 'bg-white';
  }
      ...
      <div className="space-y-2">
        {question.options.map((option, i) => (
          <button
            ...
            disabled={isAnswered}
            className={`w-full text-left p-2 border rounded-md transition-colors ${getOptionClass(option)}`}
          >
            {option}
          </button>
        ))}
...
const QuizToolView: React.FC<QuizToolViewProps> = ({ onBack }) => {
  ...
  return (
    <div className={commonViewClasses}>
      <header className="mb-10 flex-shrink-0">
        ...
        <h1 className="text-4xl font-bold text-gray-900">Notes to Quiz</h1>
        <p className="text-lg text-gray-500 mt-2">Test your knowledge by converting your notes into a practice quiz.</p>
      </header>
      ...
      <div className="flex-grow grid grid-cols-1 md:grid-cols-2 gap-8 min-h-0">
        ...
        <div className="flex flex-col">
            ...
            <div className="flex-1 w-full p-4 border border-gray-300 rounded-lg bg-gray-50 relative overflow-y-auto">
              ...
              {status === 'idle' && questions.length === 0 && <p className="text-gray-500 text-center pt-10">Your quiz will appear here.</p>}
            </div>
        </div>
      </div>
