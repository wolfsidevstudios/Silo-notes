...
import { VoiceTypingIcon, VoiceMemoIcon, TextToSpeechIcon, StopIcon, RewriteIcon, SummarizeIcon, YouTubeIcon, ImageIcon, PdfIcon, FileIcon, CloseIcon, ShareIcon } from './icons';
...
const ClassicNoteEditor: React.FC<ClassicNoteEditorProps> = ({ currentNote, onSave, gems, setGems, isGoogleConnected }) => {
  ...
  return (
    <div className="p-8 lg:p-12 h-full flex flex-col relative">
        ...
        <div className="flex items-center gap-4">
            <div className="flex items-center bg-gray-200 rounded-full p-1">
             ...
             <button
                ...
                disabled={!isGoogleConnected || isActionActive}
                className="bg-white text-black p-2 rounded-full hover:bg-gray-100 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-black disabled:bg-gray-200 disabled:text-gray-400 disabled:cursor-not-allowed border"
                title={isGoogleConnected ? "Share via Email" : "Connect Google in Settings to share"}
            >
              <ShareIcon />
            </button>
      ...
      {isShareModalOpen && <ShareViaEmailModal noteTitle={title} noteContent={content} onClose={() => setIsShareModalOpen(false)} />}
    </div>
  );
};
